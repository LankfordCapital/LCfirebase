...
import { PlusCircle, Trash2, Edit, Save, X, FileText, Users, User, FileCheck } from 'lucide-react';
...
interface Doc {
    ...
    content: string;
    createdAt: any;
    type?: 'broker' | 'borrower' | 'loan';
}

type FileType = 'broker' | 'borrower' | 'loan';
...
export default function DocsPage() {
    ...
    const [activeTab, setActiveTab] = useState<FileType>('broker');
    
    const { toast } = useToast();
    
    const fetchDocsByType = async (type: FileType) => {
        setIsLoading(true);
        const q = query(collection(db, "workforce-docs"), orderBy("createdAt", "desc"));
        const querySnapshot = await getDocs(q);
        const allDocs = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Doc[];
        const filteredDocs = allDocs.filter(doc => doc.type === type);
        setDocs(filteredDocs);
        setIsLoading(false);
    };

    useEffect(() => {
        fetchDocsByType(activeTab);
    }, [activeTab]);

    const handleNewDoc = () => {
        setCurrentDoc({ type: activeTab });
        ...
        try {
            if (currentDoc.id) {
                ...
                const docRef = await addDoc(collection(db, "workforce-docs"), {
                    ...
                    type: currentDoc.type || activeTab,
                    createdAt: new Date(),
                });
                 setDocs([{ id: docRef.id, title: currentDoc.title, content: currentDoc.content, type: currentDoc.type || activeTab, createdAt: new Date() }, ...docs]);
    ...
    return (
        <div className="space-y-6">
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as FileType)} className="w-full">
                ...
                <TabsContent value="broker" className="space-y-4">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl font-semibold">Broker Files</h2>
                ...
                <TabsContent value="borrower" className="space-y-4">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl font-semibold">Borrower Files</h2>
                        <span className="text-sm text-muted-foreground">{docs.length} documents</span>
                    </div>
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {isLoading ? (
                ...
                <TabsContent value="loan" className="space-y-4">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl font-semibold">Loan Files</h2>
                        <span className="text-sm text-muted-foreground">{docs.length} documents</span>
                    </div>
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {isLoading ? (
             ...
             <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[625px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {currentDoc.id ? 'Make changes to your document.' : `Create a new ${activeTab} document for your team.`}
                        </DialogDescription>
                    </DialogHeader>
