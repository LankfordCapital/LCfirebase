...
export interface BorrowerLoanActionsData {
  loans: LoanAction[];
  loading: boolean;
  error: string | null;
  refreshData: () => void;
...
export function useBorrowerLoanActions(): BorrowerLoanActionsData {
  ...
  const fetchLoanActions = async () => {
    try {
      ...
      console.error('Error fetching borrower loan actions:', err);
  ...
  const getMissingDocuments = (application: any): MissingDocument[] => {
    ...
    if (!docTracking.borrowerDocuments?.socialSecurityCard?.fileUrl) {
      missingDocs.push({
        id: 'ss_card',
        ...
        required: true,
        status: 'missing',
        note: docTracking.borrowerDocuments?.socialSecurityCard?.notes
      });
    }

    // Check if asset statements are missing
    if (!docTracking.borrowerDocuments?.personalAssetStatements || 
        docTracking.borrowerDocuments.personalAssetStatements.length === 0) {
      missingDocs.push({
        id: 'asset_statements',
        name: 'Asset Statements (Last 3 Months)',
        category: 'borrower',
        required: true,
        status: 'missing',
        note: docTracking.borrowerDocuments?.personalAssetStatements?.[0]?.notes
      });
    }
    ...
    if (!docTracking.propertyDocuments?.earnestMoneyDeposit?.fileUrl) {
      missingDocs.push({
        id: 'earnest_money',
        ...
        required: true,
        status: 'missing',
        note: docTracking.propertyDocuments?.earnestMoneyDeposit?.notes
      });
    }
    ...
    // Check for pending documents that might need attention
    Object.entries(docTracking.borrowerDocuments || {}).forEach(([key, doc]: [string, any]) => {
      if (doc?.status === 'pending' && doc?.fileUrl) {
        missingDocs.push({
          id: `pending_${key}`,
          name: getDocumentDisplayName(key),
          category: 'borrower',
          required: true,
          status: 'pending',
          note: doc.notes || 'Document is under review'
        });
      }
    });
